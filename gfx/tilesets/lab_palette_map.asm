	tilepal 0, GRAY, YELLOW, YELLOW, YELLOW, YELLOW, GRAY, GRAY, GRAY
	tilepal 0, WATER, WATER, WATER, GRAY, GRAY, GREEN, GREEN, RED
	tilepal 0, GRAY, YELLOW, YELLOW, YELLOW, YELLOW, GRAY, GRAY, GRAY
	tilepal 0, WATER, WATER, WATER, GRAY, GRAY, GREEN, GREEN, RED
	tilepal 0, YELLOW, YELLOW, WATER, WATER, GRAY, WATER, WATER, PRIORITY_GRAY
	tilepal 0, GRAY, GRAY, GRAY, GRAY, GRAY, GREEN, GREEN, BROWN
	tilepal 0, YELLOW, YELLOW, BROWN, BROWN, PRIORITY_GRAY, WATER, WATER, GRAY
	tilepal 0, GREEN, GREEN, GRAY, GRAY, GRAY, GRAY, GRAY, BROWN
	tilepal 0, BROWN, GRAY, GRAY, GRAY, GRAY, GRAY, GRAY, GRAY
	tilepal 0, GRAY, GRAY, GRAY, GRAY, GRAY, GRAY, GRAY, BROWN
	tilepal 0, GRAY, GRAY, GRAY, GRAY, GRAY, GRAY, GRAY, GRAY
	tilepal 0, BROWN, BROWN, GRAY, GRAY, GRAY, GRAY, GRAY, BROWN
	tilepal 0, BROWN, BROWN, BROWN, GRAY, GRAY, GRAY, GRAY, GRAY
	tilepal 0, RED, RED, GRAY, PRIORITY_GRAY, BROWN, BROWN, BROWN, BROWN
	tilepal 0, BROWN, BROWN, BROWN, BROWN, WATER, GRAY, GRAY, GRAY
	tilepal 0, GREEN, GREEN, GREEN, GREEN, BROWN, BROWN, WATER, TEXT
	tilepal 1, GRAY, BROWN, BROWN, BROWN, RED, GREEN, WATER, WATER
	tilepal 1, BROWN, BROWN, GRAY, GRAY, GRAY, GRAY, BROWN, BROWN
	tilepal 1, BROWN, BROWN, BROWN, BROWN, RED, BROWN, WATER, WATER
	tilepal 1, BROWN, BROWN, GRAY, GRAY, BROWN, BROWN, BROWN, BROWN
	tilepal 1, GRAY, GRAY, BROWN, BROWN, BROWN, BROWN, BROWN, BROWN
	tilepal 1, GRAY, GRAY, GRAY, GRAY, WATER, WATER, BROWN, BROWN
	tilepal 1, GRAY, GRAY, BROWN, BROWN, BROWN, BROWN, GRAY, GRAY
	tilepal 1, GRAY, GRAY, BROWN, BROWN, WATER, WATER, BROWN, BROWN
	tilepal 1, BROWN, GRAY, GRAY, GRAY, GRAY, GRAY, GRAY, GRAY
	tilepal 1, GRAY, GRAY, BROWN, BROWN, BROWN, BROWN, BROWN, BROWN
	tilepal 1, GRAY, GRAY, GRAY, GRAY, GRAY, GRAY, GRAY, GRAY
	tilepal 1, GRAY, GRAY, BROWN, BROWN, BROWN, BROWN, BROWN, BROWN
